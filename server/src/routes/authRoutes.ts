...
router.post('/register', validate(registerSchema), async (req, res) => {
  try {
    const { username, email, password, firstName, lastName, householdId } =
      req.body;
...
router.post('/login', validate(loginSchema), async (req, res) => {
  try {
    ...
    if (!user.password) {
      return res
        .status(401)
        .json({ message: 'Please use OAuth sign-in for this account' });
    }
...
router.get(
  ...
  authenticateToken,
  validate(householdParamsSchema),
  async (req: AuthRequest, res) => {
    try {
      ...
      if (req.user.householdId !== householdId) {
        return res
          .status(403)
          .json({ message: 'Access denied to this household' });
      }
      ...
      res
        .status(500)
        .json({ message: 'Server error fetching household members' });
    }
  }
);
