...
router.get('/', authenticateToken, async (req: AuthRequest, res) => {
  try {
    const filters = {
      ...
      dietaryTags: Array.isArray(req.query.dietaryTags)
        ? (req.query.dietaryTags as string[])
        : (req.query.dietaryTags as string),
...
router.get(
  ...
  async (req: AuthRequest, res) => {
    try {
      const metadata = await recipeService.getRecipeMetadata(
        req.user.householdId
      );
      res.json(metadata);
    } catch (error) {
      ...
      res
        .status(500)
        .json({ message: 'Server error fetching recipe metadata' });
    }
  }
);
